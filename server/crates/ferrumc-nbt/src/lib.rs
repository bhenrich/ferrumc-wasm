...
//! * **Total bytes.** `max_bytes` caps the length of the input slice itself, so
//!   it transitively bounds every read and allocation underneath it.
...
pub use read::{read_named_root, read_network_root};
