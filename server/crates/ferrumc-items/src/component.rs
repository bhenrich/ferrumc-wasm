...
pub struct ComponentTypeId(pub i32);
...
pub enum ComponentValue {
    ...
    CustomData(NbtTag),
    /// Any other component, carried as its raw *trusted* wire bytes under the
    /// given type id (no length prefix — see the crate README).
    Opaque {
...
mod tests {
    ...
    fn dangerous_and_range_classification() {
        ...
        for tid in [
            ...
            assert!(ComponentTypeId(tid).is_dangerous(), "type {tid} must strip");
        }
        // Plain typed/value components are not stripped.
        assert!(!ComponentTypeId(CUSTOM_DATA).is_dangerous());
        assert!(!ComponentTypeId(MAX_STACK_SIZE).is_dangerous());
        assert!(!ComponentTypeId(ITEM_NAME).is_dangerous());
        assert!(ComponentTypeId(0).is_in_range());
        assert!(ComponentTypeId(MAX_COMPONENT_TYPE_ID).is_in_range());
        assert!(!ComponentTypeId(96).is_in_range());
        assert!(!ComponentTypeId(-1).is_in_range());
    }
    ...
    fn patch_len_and_empty() {
        ...
        patch.added.push(ComponentValue::Unbreakable);
        patch.removed.push(ComponentTypeId(DAMAGE));
